...
import { AvatarOnlineIndicator, VerifiedTick } from "./components";
...
export const Avatar = ({
  ...
  const renderMainContent = () => {
    ...
    if (initials) {
      return (
        <span className={cn("text-quaternary", styles[size].initials)}>
          {initials}
        </span>
      );
    }
    ...
    if (PlaceholderIcon) {
      return (
        <PlaceholderIcon
          className={cn("text-fg-quaternary", styles[size].icon)}
        />
      );
    }
    ...
    return (
      placeholder || (
        <User className={cn("text-fg-quaternary", styles[size].icon)} />
      )
    );
  };
