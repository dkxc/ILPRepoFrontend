/**
 * Adapted from Untitled UI
 */
import type { ComponentProps, ComponentPropsWithRef } from "react";
import { Children, createContext, isValidElement, useContext } from "react";
import { FileIcon } from "lucide-react";
import { Search } from "lucide-react";
import { FeaturedIcon as FeaturedIconbase } from "@ui/FeaturedIcon";
...
const FileTypeIcon = ({ type = "folder", ...props }: FileTypeIconProps) => {
  return (
    <div
      {...props}
      className={cn(
    ...
    >
      <FileIcon type={type} className="size-10 drop-shadow-sm" />
    </div>
  );
};
...
const Footer = (props: ComponentPropsWithRef<"div">) => {
  return (
    <footer {...props} className={cn("z-10 flex gap-3", props.className)} />
  );
};
