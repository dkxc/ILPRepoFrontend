...
export default function DataTable<T extends Record<string, any>>({
    ...
    const [multipleFilters, setMultipleFilters] = useState<{ [key: string]: string | null }>({});
    ...
    if (enableSearch && search) {
        filteredData = filteredData.filter(row => {
            const columnsToSearch = searchColumns.length > 0 ? searchColumns : columns.map(col => col.key);
    ...
    // Pagination
    const totalPages = enablePagination ? Math.ceil(filteredData.length / currentPageSize) : 1;
    const paginatedData = enablePagination
        ? filteredData.slice((activePage - 1) * currentPageSize, activePage * currentPageSize)
        : filteredData;
        ...
        <Stack gap="md">
                            ...
                            {Object.entries(columnFilters).map(([key, options]) => {
                                ...
                                return (
                                    <Select
                                        ...
                                        onChange={(value) => setMultipleFilters(prev => ({ ...prev, [key]: value }))}
                                        clearable
                                        style={{ width: 180 }}
                                    />
                                );
            ...
            <Paper withBorder={withBorder} p={withPadding ? 'md' : 0}>
                <Box style={{ overflowX: 'auto' }}>
                    <Table striped={striped} highlightOnHover={highlightOnHover} style={tableStyle}>
                        {!hideHeader && (<thead>
                            <tr style={{ backgroundColor: headerBgColor, color: headerTextColor, ...headerStyle }}>
                                {enableSelection && (
                                    <th style={{
                                        width: 40,
                                ...
                                {columns.map((column) => (
                                    <th
                                        ...
                                        style={{
                                            ...
                                            cursor: column.sortable && enableSort ? 'pointer' : 'default',
                                        }}
                                        onClick={() => column.sortable && handleSort(column.key)}
                                    >
                                        {column.header}
                                        {enableSort && column.sortable && sortBy === column.key && (
                        ...
                        <tbody>
                            {paginatedData.length === 0 ? (
                                <tr>
                                    <td colSpan={columns.length + (enableSelection ? 1 : 0)} style={{ textAlign: 'center', padding: 40 }}>
...
export function ProjectsTableExample() {
    ...
    const data = [
        ...
        { id: 2, projectName: 'Parking System', status: 'IN PROGRESS', progress: 45 },
